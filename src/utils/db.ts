...
function registrarEjecucion(resultado: ResultadoTasas): void {
    ...
    db.run(
        `INSERT INTO ejecuciones (fecha_creacion, resultado) VALUES (?, ?)`,
        [fecha, JSON.stringify(resultado)]
    );
}
...
function obtenerUltimaEjecucion(): Ejecucion | null {
  const query = db.query<
    { id: number; fecha_creacion: string; fecha_actualizacion: string | null; fue_enviado: number; resultado: string },
    []
...
function registrarEjecucionSiCambio(resultado: ResultadoTasas): void {
  ...
  if (ultima && sonIguales(ultima.resultado, resultado)) {
    console.log("No se insertó: la información es idéntica a la última ejecución.");
    return;
  }
